...
DEFAULT_INSERT_OFFSET_X_MM = 10.5    #10.5
DEFAULT_INSERT_OFFSET_Y_MM = -23.5   #-17.5

INSERT_2X3_OFFSET_X_MM = 7.5   #10.5
INSERT_2X3_OFFSET_Y_MM = -37.5  #19.5
...
# ============================================================
# 순응제어 파라미터 (release_sequence 하강 시 사용)
# ============================================================
# 강성값 단위: 선형축 N/m, 회전축 N·m/rad
# 값이 낮을수록 해당 축이 유연하게 순응함 (힘을 덜 버팀)
#
# [Fx, Fy, Fz, Mx, My, Mz]
#   - XY(Fx, Fy): 500 → 수평 위치 유지 (블록이 옆으로 밀리지 않도록)
#   - Z(Fz)     : 400 → 목표까지 도달하되 과도한 힘은 흡수
#                       (20이면 힘을 받는 즉시 위로 밀려 목표 미달,
#                        500 이상이면 순응 없이 위치 제어와 동일해짐)
#   - 회전 3축  : 100 → 핀-홀 정렬 오차의 모멘트를 흡수하면서도
#                       과도한 틸트는 억제 (50이면 너무 흐물흐물함)
COMPLIANCE_STIFFNESS = [500, 500, 400, 100, 100, 100]

# 순응제어 하강 속도/가속도: 힘이 누적되기 전에 목표에 도달하도록
# 너무 느리면 접촉 후 외력이 누적되어 순응 방향(상방)으로 밀릴 수 있음
COMPLIANCE_VELOCITY     = 30   # mm/s  (기존 20 → 30: 외력 누적 전 목표 도달)
COMPLIANCE_ACCELERATION = 30   # mm/s² (기존 20 → 30)
...
class RobotController(Node):
    ...
    def _run_release_sequence(self, target_pos: List[float], lift_pos: List[float]) -> None:
        ...
        Note:
            ...
            순응제어(_compliance_down)를 사용하는 이유: 힘이 느껴져도 목표까지
            계속 하강하되, 규정 이상의 과부하는 순응으로 흡수해 파손을 방지하기 위함.
            (Fz 강성 400: 접촉력 존재 시에도 위치 명령 유지하며 하강 지속)
        """
        # target_pos Z에서 13.0mm 더 내려간 압입 목표 좌표 계산
        release_down_pos = self._set_z(target_pos, target_pos[2] - 27.0)
    ...
    def _compliance_down(self, pos: List[float]) -> None:
        ...
        2. set_stiffnessx로 강성 명시 적용 (Z축 매우 유연, XY 유지, 회전 완화)
        3. 느린 속도/가속도로 movel 하강 (접촉력 최소화)
        4. release_compliance_ctrl로 순응제어 해제

        Note:
            강성값 COMPLIANCE_STIFFNESS = [Fx, Fy, Fz, Mx, My, Mz]:
            - XY(Fx, Fy): 500 N/m  → 수평 위치 견고히 유지
            - Z(Fz)     : 400 N/m  → 목표까지 도달하되 과도한 힘은 흡수
                          (20이면 접촉 즉시 위로 밀려 목표 미달,
                           500 이상이면 순응 없이 위치 제어와 동일)
            - 회전 3축  : 100 N·m/rad → 핀-홀 정렬 오차의 모멘트를 흡수하되
                          과도한 틸트는 억제 (50은 너무 흐물흐물함)

            설계 의도: "힘이 느껴져도 멈추지 않고 목표까지 내려가되,
                        과도한 충격은 순응으로 흡수해 블록·보드 파손 방지"
              · Fz=400: 접촉력이 생겨도 위치 명령을 유지하며 하강 지속,
                        규정 이상의 과부하에서만 순응 방향(상방)으로 흡수.
              · 회전 강성 100: 핀 삽입 시 미세 틸트는 허용하되 안정성 확보.
              · vel/acc=30: 힘이 누적되기 전에 목표에 도달하도록 속도 소폭 증가.
        ...
        self.get_logger().info(
            f"[COMPLIANCE] 순응제어 시작 | stiffness={COMPLIANCE_STIFFNESS} "
            f"| vel={COMPLIANCE_VELOCITY} acc={COMPLIANCE_ACCELERATION}"
        )
        try:
            ret = task_compliance_ctrl(COMPLIANCE_STIFFNESS)
        ...
        # set_stiffnessx로 강성을 명시적으로 재적용: task_compliance_ctrl 단독보다
        # 실제 컨트롤러에 강성값이 확실히 반영되도록 이중 설정
        if set_stiffnessx is not None:
            try:
                set_stiffnessx(COMPLIANCE_STIFFNESS)
                self.get_logger().info(f"[COMPLIANCE] set_stiffnessx 적용: {COMPLIANCE_STIFFNESS}")
            except Exception as e:
                self.get_logger().warn(f"[COMPLIANCE] set_stiffnessx 예외 (무시하고 계속): {e}")

        time.sleep(1.0)  # DSR 컨트롤러가 순응제어 모드를 실제 적용할 때까지 대기 (0.5s는 부족할 수 있음)
        try:
            # 느린 속도/가속도로 하강: 접촉 충격 최소화 → 순응제어 효과 극대화
            movel(pos, vel=COMPLIANCE_VELOCITY, acc=COMPLIANCE_ACCELERATION)
            self.get_logger().info("[COMPLIANCE] 압입 하강 완료")
        except Exception as e:
            self.get_logger().error(f"[COMPLIANCE] movel 예외: {e}")
        finally:
            try:
