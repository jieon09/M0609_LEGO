...
class RobotAction(str, enum.Enum):
    """로봇 동작 단계 열거형. DB Enum 컬럼과 1:1 매핑."""
    pick            = "pick"
    place           = "place"
...
class Inventory(Base):
    """레고 블럭 재고 테이블. 싱글톤 행(1행)으로 운영한다."""

    __tablename__ = "inventory"

    inventory_id: uuid.UUID = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    red_2x2:      int       = Column(Integer, nullable=False, default=0)
    red_2x3:      int       = Column(Integer, nullable=False, default=0)
    blue_2x2:     int       = Column(Integer, nullable=False, default=0)
    blue_2x3:     int       = Column(Integer, nullable=False, default=0)
    yellow_2x2:   int       = Column(Integer, nullable=False, default=0)
    yellow_2x3:   int       = Column(Integer, nullable=False, default=0)
    total_blocks: int       = Column(Integer, nullable=False, default=0)
...
class Task(Base):
    """고객 주문 작업 테이블. end_at이 None이면 진행 중 상태."""

    __tablename__ = "task"

    id          = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    customer_id = Column(String(200), nullable=True)
    brick_path  = Column(JSONB, nullable=True)   # 배치할 블럭 경로 정보 (JSON)
    img_path    = Column(String(500), nullable=True)
    created_at  = Column(DateTime(timezone=True), server_default=func.now())
    end_at      = Column(DateTime(timezone=True), nullable=True)  # None이면 미완료
...
class RobotLog(Base):
    """로봇 동작 단계별 로그. 관절각(deg)·TCP(mm) 스냅샷을 함께 저장한다."""
    ...
    # TCP 위치: robot base 좌표계, mm / deg 단위
    tcp_x  = Column(Float, nullable=True)  # mm
    tcp_y  = Column(Float, nullable=True)  # mm
    tcp_z  = Column(Float, nullable=True)  # mm
    tcp_rx = Column(Float, nullable=True)  # deg
    tcp_ry = Column(Float, nullable=True)  # deg
    tcp_rz = Column(Float, nullable=True)  # deg
