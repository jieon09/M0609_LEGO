...
def detect_line_color(img: np.ndarray) -> str:
    ...
    v_channel = hsv[:,:,2]
    dark_mask = v_channel < 100
    if not np.any(dark_mask):
        print("[색상 감지] 어두운 픽셀 없음 → 기본값 R")
        return 'R'

    h_vals = hsv[:,:,0][dark_mask]  # Hue 범위: 0~180 (OpenCV 기준)
    s_vals = hsv[:,:,1][dark_mask]  # Saturation

    # 채도 평균 < 40 이면 무채색(검정/회색) — 유채색 판단 불가
    mean_s = np.mean(s_vals)
    if mean_s < 40:
        print(f"[색상 감지] 무채색 (채도:{mean_s:.0f}) → 기본값 R")
        return 'R'

    mean_h = np.mean(h_vals)
    print(f"[색상 감지] 평균 Hue:{mean_h:.1f} 채도:{mean_s:.1f}")
...
def downsample_24x24(
    ...
    """colored 맵을 24×24 레고 그리드로 다운샘플링하고 밝기 맵 계산.
    ...
    Args:
        ...
        gray_img: 원본 그레이스케일 이미지 (밝기 맵 계산용). None이면 z_layers 계산 안 함

    Returns:
        (grid, brightness_map) 튜플
        brightness_map[r*24+c] = 0~255 (셀 최소 밝기, 낮을수록 진한 선)
    """
    CS = SZ / 24  # 셀 하나의 픽셀 크기
    grid: list[str] = []
    brightness_map: list[int] = []
    ...
    for r in range(24):
        for c in range(24):
            ...
            if total==0:
                grid.append('.'); brightness_map.append(255); continue
            er=np.sum(cell==1)/total; ir=np.sum(cell==2)/total
            # 0.06: 셀의 6% 이상이 엣지이면 외곽선 브릭으로 판단
            if   er>0.06:
                grid.append(outline_color)
            # 0.20: 셀의 20% 이상이 내부이면 채우기 브릭으로 판단
            elif ir>0.20 and fill_color!='.':
                grid.append(fill_color)
            else:
                grid.append('.')

            # 셀 내 원본 그레이 최솟값 = 가장 어두운 픽셀 밝기 → 선 굵기 추정에 사용
            if gray_img is not None:
                cell_gray = gray_img[r0:r1, c0:c1]
                brightness_map.append(int(np.min(cell_gray)))
            else:
                brightness_map.append(128)

    return grid, brightness_map
...
# ── 그리디 브릭 변환 (R/B/Y 만, z_layers 포함) ───────────────────────
def make_bricks(
    grid: list[str],
    brightness_map: Optional[list[int]] = None,
    dark_threshold: int = 100,
    max_z: int = 2,
) -> tuple[list[dict], int]:
    """24×24 그리드를 2×2, 2×3, 3×2 브릭으로 그리디 변환 (z_layers 포함).

    처리 흐름:
    1. 좌상→우하 순으로 스캔
    2. 미사용 셀에서 가장 큰 브릭(2×3 → 3×2 → 2×2) 우선 배치
    3. 브릭 영역 평균 밝기로 z_layers 결정

    Args:
        grid: 24×24 색상 코드 리스트
        brightness_map: 셀별 밝기값 (0=완전검정, 255=완전흰색). None이면 z_layers=1 고정
        dark_threshold: 이 값 미만이면 진한 선 → z_layers=max_z (기본 100)
        max_z: 진한 선일 때 쌓을 층수 (기본 2층)
    ...
    def get_z_layers(r: int, c: int, w: int, h: int) -> int:
        """브릭 영역의 평균 밝기로 z_layers 결정 — 어두울수록 더 많은 레이어."""
        ...
        for dr in range(h):
            for dc in range(w):
                idx = (r+dr)*N + (c+dc)
                if idx < len(brightness_map):
                    vals.append(brightness_map[idx])
        if not vals:
            return 1
        avg_brightness = sum(vals) / len(vals)
        return max_z if avg_brightness < dark_threshold else 1
    ...
    def place(r: int, c: int, w: int, h: int, color: str) -> None:
        ...
        z = get_z_layers(r, c, w, h)
        bricks.append({'row':r,'col':c,'width':w,'height':h,'color':color,'z_layers':z})
...
def process(
    ...
    """이미지 → 레고 브릭 JSON 변환 메인 파이프라인 (z_layers 포함 버전).

    처리 흐름:
    0. 종이 검출 + 원근 변환 (no_paper 옵션 시 스킵)
    1. EdgeDetector 이진화
    2. Erode / 3. Dilate / 4. Sobel / 5. Flood Fill
    6. 24×24 다운샘플 (gray 이미지로 밝기 맵 계산)
    7. 소규모 클러스터 제거
    8. 그리디 브릭 변환 (z_layers 포함) + JSON 저장

    Args:
        dark_thresh: 셀 밝기가 이 값 미만이면 진한 선 → z_layers=max_z_layers
        max_z_layers: 진한 선일 때 쌓을 층수 (기본 2)
    ...
    # Step 6: 24×24 다운샘플 + 밝기 맵 계산 (gray 이미지 함께 전달)
    gray_img = cv2.cvtColor(warped, cv2.COLOR_BGR2GRAY)
    grid, brightness_map = downsample_24x24(colored, SZ, outline_color, fill_color, gray_img)

    # Step 7: 클러스터 제거
    grid     = remove_small_clusters(grid, min_cluster)
    grid_vis = make_grid_vis(grid)

    # Step 8: 브릭 변환 (R/B/Y 만, z_layers 포함)
    bricks, dropped = make_bricks(grid, brightness_map,
...
def batch_process(
    ...
    Note:
        이 버전(v4)은 no_show 옵션이 없어 항상 시각화 창을 표시함
    """
    import glob
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument('--dark-thresh', type=int, default=100,
                   help='셀 밝기 임계값 (0~255, 미만이면 진한 선 → z_layers=max-z, 기본 100)')
    p.add_argument('--max-z',       type=int, default=2,
                   help='진한 선일 때 쌓을 층수 (기본 2)')
    return p.parse_args()
